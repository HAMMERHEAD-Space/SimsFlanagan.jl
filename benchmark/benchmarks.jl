...
SUITE["Kepler"]["propagate_circular"] = @benchmarkable SimsFlanagan.kepler_propagate(
    $r0_bench, $v0_bench, 3600.0, $μ_bench
)
SUITE["Kepler"]["propagate_long"] = @benchmarkable SimsFlanagan.kepler_propagate(
    $r0_bench, $v0_bench, $Δt_bench, $μ_bench
)

# Segment propagation
SUITE["Segment"] = BenchmarkGroup()
SUITE["Segment"]["Spacecraft"] = @benchmarkable SimsFlanagan.propagate_segment(
    $r0_bench, $v0_bench, $mass_bench, $throttle_bench, $Δt_bench, $μ_bench, $sc_bench
)
SUITE["Segment"]["SEPSpacecraft"] = @benchmarkable SimsFlanagan.propagate_segment(
    $r0_bench, $v0_bench, $mass_bench, $throttle_bench, $Δt_bench, $μ_bench, $sep_bench
)
SUITE["Segment"]["SolarSail"] = @benchmarkable SimsFlanagan.propagate_segment(
    $r0_bench, $v0_bench, $mass_bench, $throttle_bench, $Δt_bench, $μ_bench, $sail_bench
)

# Leg propagation
const throttles_bench_4 = [SVector{3}(0.1*i, 0.05*i, 0.02*i) for i in 1:4]
...
SUITE["Leg"]["Spacecraft_4seg"] = @benchmarkable SimsFlanagan.propagate_leg(
    $r0_bench, $v0_bench, $mass_bench, $throttles_bench_4, $Δt_segments_bench, $μ_bench, $sc_bench
)

# Problem construction
SUITE["Problem"] = BenchmarkGroup()
SUITE["Problem"]["construction_4seg"] = @benchmarkable simsflanagan_problem(
    $r0_bench, $v0_bench, $rf_bench, $vf_bench, $tof_bench, $μ_bench, $sc_bench;
    n_segments = 4
)
...
const prob_bench_10 = simsflanagan_problem(
    r0_bench, v0_bench, rf_bench, vf_bench, tof_bench, μ_bench, sc_bench;
    n_segments = 10, verbosity = 0
)
const throttles_bench_mismatch = [SVector{3}(0.1, 0.1, 0.0) for _ in 1:4]
const throttles_bench_10 = [SVector{3}(0.1*i/10, 0.05*i/10, 0.02*i/10) for i in 1:10]
SUITE["Mismatch"] = BenchmarkGroup()
SUITE["Mismatch"]["compute_4seg"] = @benchmarkable compute_mismatch(
    $prob_bench_4, $throttles_bench_mismatch
)
SUITE["Mismatch"]["compute_10seg"] = @benchmarkable compute_mismatch(
    $prob_bench_10, $throttles_bench_10
)

# Scaled mismatch constraints (used in solve)
SUITE["Constraints"] = BenchmarkGroup()
SUITE["Constraints"]["scaled_mismatch_4seg"] = @benchmarkable scaled_mismatch_constraints(
    $prob_bench_4, $throttles_bench_mismatch
)
SUITE["Constraints"]["scaled_mismatch_10seg"] = @benchmarkable scaled_mismatch_constraints(
    $prob_bench_10, $throttles_bench_10
)
SUITE["Constraints"]["throttle_magnitude_4seg"] = @benchmarkable SimsFlanagan.throttle_magnitude_constraints(
    $throttles_bench_mismatch
)
SUITE["Constraints"]["throttle_magnitude_10seg"] = @benchmarkable SimsFlanagan.throttle_magnitude_constraints(
    $throttles_bench_10
)
...
SUITE["Spacecraft"]["compute_thrust"] = @benchmarkable SimsFlanagan.compute_thrust(
    $sc_bench, $r0_bench, 0.5
)

# SEP operations
SUITE["SEP"] = BenchmarkGroup()
SUITE["SEP"]["compute_thrust_1AU"] = @benchmarkable SimsFlanagan.compute_thrust(
    $sep_bench, SVector{3}(1.495978707e8, 0.0, 0.0), 0.5
)
SUITE["SEP"]["compute_thrust_2AU"] = @benchmarkable SimsFlanagan.compute_thrust(
    $sep_bench, SVector{3}(2.99e8, 0.0, 0.0), 0.5
)

# Solar sail operations
SUITE["SolarSail"] = BenchmarkGroup()
SUITE["SolarSail"]["characteristic_acceleration"] = @benchmarkable characteristic_acceleration($sail_bench)
SUITE["SolarSail"]["compute_thrust_1AU"] = @benchmarkable SimsFlanagan.compute_thrust(
    $sail_bench, SVector{3}(1.495978707e8, 0.0, 0.0), 0.5
)

# Total ΔV computation
SUITE["TotalDV"] = BenchmarkGroup()
SUITE["TotalDV"]["compute_4seg"] = @benchmarkable compute_total_Δv(
    $throttles_bench_4, $Δt_segments_bench, $sc_bench
)

# Initial guess generation
SUITE["InitialGuess"] = BenchmarkGroup()
SUITE["InitialGuess"]["zero"] = @benchmarkable SimsFlanagan.generate_initial_guess(
    $prob_bench_4, ZeroGuess()
)
SUITE["InitialGuess"]["random"] = @benchmarkable SimsFlanagan.generate_initial_guess(
    $prob_bench_4, RandomGuess()
)
SUITE["InitialGuess"]["constant"] = @benchmarkable SimsFlanagan.generate_initial_guess(
    $prob_bench_4, ConstantGuess(direction = [1, 0, 0], magnitude = 0.5)
)
SUITE["InitialGuess"]["lambert"] = @benchmarkable SimsFlanagan.generate_initial_guess(
    $prob_bench_4, LambertGuess()
)

# Full solve benchmarks
SUITE["Solve"] = BenchmarkGroup()
SUITE["Solve"]["4seg_zero_guess"] = @benchmarkable solve(
    $prob_bench_4; initial_guess_strategy = ZeroGuess(), max_iter = 100
)
SUITE["Solve"]["4seg_random_guess"] = @benchmarkable solve(
    $prob_bench_4; initial_guess_strategy = RandomGuess(), max_iter = 100
)
SUITE["Solve"]["4seg_lambert_guess"] = @benchmarkable solve(
    $prob_bench_4; initial_guess_strategy = LambertGuess(), max_iter = 100
)
SUITE["Solve"]["10seg_lambert_guess"] = @benchmarkable solve(
    $prob_bench_10; initial_guess_strategy = LambertGuess(), max_iter = 100
)
...
SUITE["Helpers"]["throttles_to_vector_4"] = @benchmarkable SimsFlanagan.throttles_to_vector(
    $throttles_bench_mismatch
)
SUITE["Helpers"]["vector_to_throttles_4"] = @benchmarkable SimsFlanagan.vector_to_throttles(
    $x_bench_flat, 4
)
SUITE["Helpers"]["get_canonical_scales"] = @benchmarkable SimsFlanagan.get_canonical_scales(
    $prob_bench_4
)
SUITE["Helpers"]["compute_segment_masses_4"] = @benchmarkable SimsFlanagan.compute_segment_masses(
    $prob_bench_4, $throttles_bench_mismatch, $Δt_segments_bench
)
